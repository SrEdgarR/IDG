...
> **Estado actual: aplicación de desarrollo (fase 07, en revisión).** Permite descargas HTTP/HTTPS reales, colas con horarios, reglas de carpeta/categoría, importación de enlaces, búsqueda e historial. La extensión Chromium de desarrollo pasó una prueba básica de traspaso con archivos locales; su matriz de fallos y compatibilidad sigue pendiente. No hay instalador, versión publicada ni extensión en tiendas. No se presenta como una versión estable para uso habitual.
...
Solo para desarrollo: sigue la [guía de compilación y carga local](docs/DESARROLLO.md) y el [recorrido desde la ventana con un archivo local](docs/APP_DEVELOPMENT.md). Abrir IDG inicia su motor validado; el popup de la extensión muestra **Conectado** tras un intercambio real. **Reconectar** comprueba el enlace. X oculta la ventana y conserva las descargas; **Salir completamente** confirma y guarda el estado antes de detenerlo. La extensión Chromium solicita aparte el permiso para detectar descargas y solo propone enlaces seguros según la [guía de prueba de Chromium](docs/CHROMIUM_DEVELOPMENT.md); su matriz de fallo/fallback todavía está pendiente. Esta carga local no es una instalación final para usuarios.
...
| Navegadores | Extensión para Chrome, Edge, Firefox y otros navegadores contemplados en la matriz de pruebas; sin Safari | Puente probado en Chrome for Testing y Firefox; traspaso básico Chromium verificado con fixture, matriz de fallos/Chrome/Edge pendiente |
