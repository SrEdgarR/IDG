...
> **Estado actual: aplicación de desarrollo (fase 08, EN_CURSO).** Permite descargas HTTP/HTTPS reales, colas con horarios, reglas de carpeta/categoría, importación de enlaces, búsqueda e historial. La descarga desde el navegador requiere elegir explícitamente un enlace HTTP(S) directo, público y repetible y confirmar la solicitud en IDG. AutoPick sigue siendo un requisito del producto, pero la captura automática de descargas ya iniciadas está deshabilitada; esas descargas continúan en el navegador. Las sesiones autenticadas no son compatibles y no se transfieren cookies ni credenciales. Firefox tiene un build y popup propios para enlaces directos, pero su traspaso real está pendiente porque el registro Native Messaging actual apunta a otro checkout. No hay instalador, versión publicada ni extensión en tiendas; no se presenta como una versión estable para uso habitual.
...
Solo para desarrollo: sigue la [guía de compilación y carga local](docs/DESARROLLO.md) y el [recorrido desde la ventana con un archivo local](docs/APP_DEVELOPMENT.md). Abrir IDG inicia su motor validado; el popup muestra **Conectado** solo tras un intercambio real. **Reconectar** comprueba el enlace. X oculta la ventana y conserva las descargas; **Salir completamente** confirma y guarda el estado antes de detenerlo. Para transferir un archivo, elige explícitamente un enlace HTTP(S) directo, público y repetible y confirma la solicitud en IDG. La captura automática está deshabilitada; no se solicita permiso `downloads` para activarla. AutoPick sigue siendo un requisito del producto. Las sesiones autenticadas no son compatibles y no se transfieren cookies ni credenciales. Firefox incluye un popup con entrada de URL directa y trabajos reales, pero la prueba de transferencia quedó bloqueada porque su host registrado pertenece a otro checkout. Los permisos y gestos con los manifiestos normales siguen pendientes de comprobación. Esta carga local no es una instalación final para usuarios.
