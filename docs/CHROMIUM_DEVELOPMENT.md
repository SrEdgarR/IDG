...
Dos pulsaciones rápidas sobre la misma propuesta pendiente reutilizan su ID; no abren dos trabajos. Si se pierde la respuesta del host, la extensión conserva la propuesta hasta poder consultar su resultado: pulsa «Reconectar» después de iniciar de nuevo el motor. Para enlaces elegidos manualmente, la extensión no abre otra descarga por su cuenta mientras desconoce si IDG los aceptó; el aviso permite abrir el enlace original en Chromium si prefieres continuar allí. Una descarga que ya comenzó en Chromium sigue siendo suya hasta que IDG confirme bytes guardados. Si IDG informa que el trabajo falló, la propuesta se resuelve y el original activo continúa en Chromium. Revisa ambas listas antes de repetir un enlace cuyo estado sea incierto.

La prueba sin permiso opcional es `node scripts/test-extension-shell.mjs` después de construir la extensión e instalar Chromium de Playwright. `node scripts/test-chromium-capture.mjs` usa un perfil aislado y un manifiesto de prueba con `downloads` y acceso limitado al origen local `127.0.0.1` ya concedidos; verifica el traspaso observado, Pausar/Reanudar desde el popup, dos clics de un enlace elegido y la pérdida del motor antes de aceptar, con archivos y hashes reales. Abrir el popup como pestaña de automatización no concede `activeTab`: esta prueba no sustituye el gesto de pulsar el icono ni el diálogo de permiso en un Chrome interactivo. Tampoco cubre la pérdida del host después de aceptar ni una terminación real del worker. El manifiesto normal no añade acceso permanente a sitios. La matriz gráfica/nativa sigue requiriendo el host registrado para esta misma copia y un único motor IDG. `scripts/Check.ps1 -Integration` no debe atribuirse al CI: el workflow remoto solo compila, ejecuta pruebas generales y comprueba la carga del popup sin host. Consulta [IMPLEMENTATION_STATUS.md](IMPLEMENTATION_STATUS.md) para resultados y pendientes concretos.
