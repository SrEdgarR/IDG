...
En la reanudación de fase 07, `test:coverage` ejecutó **37 pruebas** con el mismo alcance y umbrales: **527/1929 líneas (27,31 %), 100/659 funciones (15,17 %) y 469/1854 ramas (25,29 %)** TypeScript propias. En `worker.ts`, la medición fue **83,87 % líneas, 71,42 % funciones y 65,72 % ramas**; `popup.ts` llegó a **63,41 % líneas** con regresiones que exigen conservar botones activos/propuestas y un campo de exclusión editado durante actualizaciones de progreso. Los casos nuevos comprueban permiso opcional, exclusiones, doble envío, respuesta perdida, recuperación de propuesta y trabajo fallido, requisito de bytes durables y estabilidad de los controles del popup. Esta medición no reemplaza la tabla histórica antes/después ni se mezcla con Rust; los archivos sin tests siguen incluidos.
