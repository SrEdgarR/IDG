...
| Chromium: política, mensajería y recuperación | Directa en worker/bridge; integración real en `test-chromium-capture.mjs` para enlace explícito, host/worker reiniciados y cierre del navegador. Las descargas observadas no se traspasan: `DownloadItem` no informa método/cuerpo; el fallback que conserva Chromium está probado. Permiso interactivo, icono y menú contextual siguen pendientes. Firefox conserva el puente de fase 01 y su prueba de integración. |
