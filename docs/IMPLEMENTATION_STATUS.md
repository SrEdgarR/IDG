...
## Prueba manual de cancelación de energía simulada — 2026-09-24

**BLOQUEADA / PENDIENTE.** La observación de la interfaz y una consulta general del runtime no permitieron demostrar desde la sesión de prueba la secuencia completa de activación simulada y cancelación. No se cambió ninguna preferencia, no se creó ninguna activación de energía y no queda ninguna acción de esta prueba que cancelar. No se repite este recorrido ni se añaden diagnósticos solo para cerrarlo. Las pruebas automáticas de energía descritas arriba conservan su propio alcance; no convierten esta comprobación manual en aprobada.

## Fase 07 — extensión Chromium (EN_CURSO)

Rama de trabajo `feat/07-extension-chromium`, aislada del checkout con modificaciones concurrentes. La base es el último commit publicado de fase 06 (`7ee3af0`); PR #6 sigue abierta y no se fusiona. No se publica la extensión ni una release.

Contrato nuevo en `idg-protocol`, `idg-runtime` y Tauri: el host nativo tiene una lista cerrada de órdenes para estado público, modo AutoPick y solicitudes de captura. No puede detener el motor ni acceder al historial privado. El runtime presenta una solicitud efímera al escritorio; Nueva descarga solo confirma si el usuario declara que el GET público admite repetición. El trabajo se guarda en estado diferido y el worker consulta su aceptación durable antes de transferir. La extensión mantiene el original de Chromium hasta observar bytes durables en IDG; URLs con consulta, fragmento, credenciales o tamaño/estado inseguros permanecen en Chromium. El popup toma trabajos y preferencias del runtime, con permisos de detección opcionales, ignorados locales, suspensión temporal, propuestas del modo Preguntarme, menú contextual de enlace/vídeo y selección explícita de enlaces de la página.

**Verificado automáticamente:** `scripts/Check.ps1` general terminó con salida 0 en Windows: formato, Clippy estricto, pruebas Rust, tipos, TypeScript, builds Tauri/extensión y regresiones HTTP/runtime/segmentación. El test de shell cargó worker y popup en Chromium sin el permiso opcional y detectó un fallo de arranque que se corrigió; no confirma un diálogo interactivo de permiso. `test-chromium.mjs` probó extensión → host → pipe → runtime, desconexión/reconexión y cierre del navegador con el motor aún activo. Estas pruebas usan perfiles y datos aislados, no son revisión humana.

**Integración real automatizada:** `test-chromium-capture.mjs` abrió IDG en Tauri/WebView2 y Chromium con Native Messaging, aceptó una descarga observada tras persistirla, esperó bytes durables antes de cancelar el original y verificó un archivo IDG de 8 MiB, su SHA-256 y un solo trabajo. Al cancelar «Nueva descarga», el archivo original terminó en Chromium sin otro trabajo IDG. «Preguntarme» necesitó elegir la propuesta; «Usar el navegador» no abrió el diálogo; el modo persistió al reabrir el popup. La excepción `.bin` persistió y conservó el archivo en Chromium. El recorrido ampliado previsualizó un solo enlace desde una página HTML local, lo eligió en el popup y completó `direct.bin` en IDG con hash correcto, sin descarga paralela en Chromium ni trabajo adicional. El perfil aislado concede `downloads` y acceso **solo a `127.0.0.1`** en su manifiesto de prueba; el manifiesto normal mantiene el permiso opcional y `activeTab`. Una primera repetición reveló una carrera del arnés al no esperar la respuesta de guardado del modo y otra intentó rellenar un control dentro de `<details>` cerrado; se corrigió el arnés. La previsualización falló al abrir el popup como pestaña ordinaria sin la concesión de `activeTab` del gesto real; la repetición con permiso de origen de fixture pasó, sin atribuirle la prueba de ese gesto. La ventana había generado además un ciclo de consultas al refrescar cada evento de clientes; se corrigió en `App.tsx` y la regresión verifica un recuento acotado. La revisión encontró que una propuesta entrante podía sustituir el contexto de un formulario manual ya abierto; ahora espera a que este cierre. La integración final abrió primero un formulario manual, recibió la propuesta sin cambiar su URL/botón y mostró la propuesta solo después de cerrarlo.

**PENDIENTE para completar la matriz 07:** permiso concedido/rechazado mediante el diálogo real de Chrome; gesto del icono que concede `activeTab` y menú contextual; ignorados por sitio/MIME/tamaño/desconocido; dos clics; caída del worker antes y después de aceptar; pérdida del host durante traspaso; cierre del navegador durante la transferencia; sesiones autenticadas y redirecciones; compatibilidad específica de Chrome/Edge. La prueba de carga sin permiso no resolvió un diálogo headless. Un intento de reinicio de la extensión completa dejó el complemento inaccesible en Chromium headless; el protocolo de esa versión tampoco expuso `ServiceWorker.enable` para detener solo el worker. No se atribuye una prueba de recuperación de worker que no se logró ejecutar. El traspaso probado es para GET público y repetible, sin cookies ni URL firmada. La PR permanece en borrador y no se integra ni publica en tiendas hasta resolver los casos que puedan perder o duplicar una descarga.

El registro temporal de Chrome/Chromium/Edge se restauró y se comprobó contra los tres valores anteriores; no quedó activo ningún proceso IDG de la rama aislada. El test de shell posterior volvió a pasar sin resolver el diálogo opcional en headless. No se alteraron perfiles personales para esta integración.

**SIGUIENTE_PASO:** revisar CI del HEAD de la PR 07 y ejecutar la matriz de fallo/fallback con Chrome/Edge interactivos y perfiles aislados; corregir cualquier pérdida o duplicado, repetir pruebas afectadas y solicitar revisión antes de integrar. La cancelación manual de energía simulada permanece BLOQUEADA / PENDIENTE como limitación independiente de fase 06; no se repite ni se implementan diagnósticos para cerrarla.
...
**Siguiente paso registrado en el pulido original (histórico):** cerrar la instancia abierta cuando el propietario termine de usarla, compilar el escritorio nativo y recorrer visualmente la interfaz y sus estados en WebView2; verificar el popup real en Chromium/Firefox. Mantener por separado la revisión pendiente de fase 06 y su PR #6.

## Integración del rediseño en la rama 07 — 2026-09-24

**VERIFICADO automáticamente en el código integrado.** Se unió `codex/workspace-ui-redesign` (`88d77e1`) a `feat/07-extension-chromium` sin modificar el archivo de referencia local sin seguimiento. El popup Chromium conserva los controles y la lógica real de fase 07; la cabecera y los estilos nuevos muestran `Conectando`, `Conectado` y `Desconectado` según la respuesta del worker/host, no por un estado simulado. El escritorio incorpora la navegación estrecha y los tokens visuales. `pnpm check`, `pnpm build`, `pnpm test:ui` (41 capturas reales de la galería y el popup), `scripts/Check.ps1`, `test-extension-shell.mjs` y `test-chromium-capture.mjs` terminaron con salida 0. La integración abrió Tauri/WebView2 y Chromium mediante Native Messaging, verificó los archivos y sus hashes; el registro temporal del host se restauró. La prueba visual automatizada no equivale a una revisión humana ni a pulsar el icono real de la extensión.

**SIGUIENTE_PASO vigente:** comprobar el CI del commit de integración de la PR #7 y continuar la matriz de fallo/fallback pendiente de fase 07 antes de solicitar integración. Revisar físicamente la apariencia en WebView2 y el popup real cuando corresponda. La cancelación manual de energía simulada de fase 06 sigue BLOQUEADA / PENDIENTE. No fusionar PR #6 o #7 ni publicar la extensión sin autorización.

## Etapa transversal de pruebas — 2026-09-25 (VERIFICADO localmente; CI del HEAD en la PR)

Por instrucción posterior del propietario, el desarrollo de nuevas fases queda **PAUSADO**. Esta sección sustituye los siguientes pasos de fase indicados arriba: no reanudar 07 ni iniciar 08 automáticamente. La etapa usa `test/cobertura-y-regresiones`, basada en `b6b4668189d595f9a7fd490653036c0c455769f6` de `feat/07-extension-chromium` (fase 07 más rediseño), en un worktree separado. La PR #6 y la PR #7 permanecen sin fusionar; `main` no contiene ese código. El checkout visual anterior y los procesos IDG abiertos no se alteran.

La línea base ejecutada en un checkout aislado del commit base pasó 52 pruebas Rust, cuatro tests Node de modelo/importación, comprobación de tipos, build del frontend, galería Playwright y carga shell Chromium; esta última no resuelve el permiso opcional en headless. Los recorridos de procesos con runtime único no se repitieron en esa línea base porque había una instancia activa ajena a este worktree. [TESTING](TESTING.md) separa niveles, métricas, exclusiones y huecos. La cancelación manual de energía simulada sigue **BLOQUEADA / PENDIENTE**, sin acción de energía creada ni razón para repetirla.

Sobre el código final, `Check.ps1` pasó completo en Windows: formato, Clippy estricto, 69 pruebas Rust del workspace, tipos, cuatro pruebas Node conservadas, 29 unitarias/componente/worker Vitest, builds, runtime/Native Messaging a nivel de proceso, HTTP y segmentación. También pasaron `test:coverage`, la galería Playwright (41 capturas) y el shell Chromium; estas dos últimas no acreditan el diálogo de permiso opcional ni el host real en navegador. El CI ordinario no ejecuta `-Integration`; esa matriz gráfica y la cancelación de energía simulada permanecen pendientes. La cobertura medida de todo el código propio incluido en `src` subió de 44,14 % a 51,84 % de líneas Rust y de 4,20 % a 21,16 % de líneas TypeScript; las métricas, denominadores y limitaciones se detallan por módulo y lenguaje en [TESTING](TESTING.md). Los tres defectos demostrados por regresiones y corregidos fueron la extensión codificada que eludía la política Chromium, un cierre tardío de watcher que desconectaba la sesión restablecida, y la aceptación de FTP en el formulario pese al contrato HTTP/HTTPS del motor.

**SIGUIENTE_PASO vigente:** revisar la PR dedicada de pruebas y el CI de su HEAD antes de cualquier integración; conservar los huecos acotados de [TESTING](TESTING.md) sin convertirlos en una nueva fase automática. No fusionar PR ni publicar extensión/release. Las nuevas fases solo se reanudan con autorización explícita del propietario.

## Cierre focalizado de la etapa de pruebas — 2026-09-25 (VERIFICADO localmente)

La [PR #8](https://github.com/SrEdgarR/IDG/pull/8) sigue basada en `feat/07-extension-chromium`; las fases del producto continúan **PAUSADAS**. El código del producto probado parte de `f3879cc678ce510d932ce11e4b74e70ed5e5ceae`. Este cierre amplía solo tres arneses existentes; no modifica la implementación, dependencias, exclusiones ni umbrales de cobertura.

- [Tauri/WebView2 y HTTP local](../scripts/test-app-download.mjs): «Nueva descarga» rechazó `ftp:` y `ftps:` con error en el campo URL, sin crear trabajo ni petición HTTP. A continuación aceptó una URL HTTP local, completó el archivo y su SHA-256 coincidió con el fixture. El arnés completo pasó con salida 0.
- [Chromium con host y runtime reales](../scripts/test-chromium-capture.mjs): con `exe` guardada como extensión ignorada, las descargas locales `ignored.exe` y `encoded%2Eexe` permanecieron fuera de las propuestas y trabajos IDG. Sin quitar esa excepción, `allowed.bin` sí abrió la propuesta, se aceptó y completó en IDG con hash correcto; el recorrido de enlace directo y fallback también pasó. Se usaron perfil, datos y servidor locales aislados; los bytes servidos bajo nombre `.exe` eran el fixture inocuo y no se ejecutaron.
- [Reconexión Chromium](../scripts/test-chromium.mjs): pulsar Reconectar con el runtime activo conservó el estado en línea y el mismo runtime tras cerrarse la conexión anterior; cerrar/reabrir el popup, detener/reiniciar el motor y reconectar también pasaron. La [regresión determinista del callback tardío](../apps/extension/src/worker.test.ts) sigue en Vitest; el recorrido real la complementa y no demuestra por sí solo una temporización específica del callback.

En esta ejecución pasaron `pnpm check`, `pnpm test:unit` (29/29), `pnpm test:coverage` (29/29; TypeScript 21,16 % líneas, 12,42 % funciones y 21,46 % ramas), las builds de escritorio, motor y extensión, y los tres arneses anteriores. La cobertura Rust no se volvió a medir porque no hubo cambio Rust: conserva la medición y el aviso sobre tests `#[cfg(test)]` inline de [TESTING](TESTING.md). El primer intento del arnés Chromium no pudo arrancar por falta del binario Playwright en este worktree; la repetición usó la instalación de navegador de pruebas ya disponible y pasó. El registro Chromium se restauró a su valor previo tras cada ejecución; no quedó ningún proceso IDG de prueba activo. Las capturas históricas de fase 05 generadas durante el arnés se devolvieron a su versión anterior para no sustituir esa evidencia. No se tocaron los dos perfiles temporales cuya limpieza fue rechazada.

El CI anterior de `f3879cc` pasó los jobs `ui`, `portable` y `windows` en la [ejecución 36148509977](https://github.com/SrEdgarR/IDG/actions/runs/36148509977); esa ejecución no acredita el commit documental y de arneses de este cierre. El CI ordinario tampoco ejecuta Tauri ni Native Messaging real. La cancelación manual de energía simulada sigue **BLOQUEADA / PENDIENTE** y no se repitió. Los huecos de fase 07 y la matriz ampliada descritos en [TESTING](TESTING.md) permanecen como trabajo incremental de sus propias fases, sin bloquear la revisión de esta PR de pruebas.

**SIGUIENTE_PASO vigente:** revisión del propietario de la PR #8, comprobando el CI de su HEAD en GitHub antes de cualquier integración y conservando su base en `feat/07-extension-chromium`. No fusionar ni reanudar fases, energía o publicación sin nueva autorización.

## Reanudación autorizada de fase 07 — 2026-09-25 (EN_CURSO)

La pausa transversal terminó por autorización del propietario. La [PR #8](https://github.com/SrEdgarR/IDG/pull/8), con HEAD `2c37710fb0be9a0cfe5a79b11f8b2bc7cb20e2ca`, se fusionó **solo** en `feat/07-extension-chromium` mediante `98cc44e7026c44edca81b6cdfe1226162be19e30`: seis comprobaciones del HEAD estaban aprobadas, no había conflictos y `main` no cambió. Las PR #6 y [#7](https://github.com/SrEdgarR/IDG/pull/7) conservan sus bases y siguen sin fusionar. El inventario y las métricas históricas de la etapa de pruebas permanecen en [TESTING](TESTING.md).

**Implementado y probado en el worker Chromium:** las solicitudes simultáneas del mismo enlace o descarga observada comparten una sola preparación; una respuesta perdida del host conserva el ID y la propuesta local hasta que el runtime confirme aceptación o rechazo. No se lanza automáticamente otra descarga de Chromium cuando el estado del motor es incierto. Al reconectar, se consulta el trabajo persistido; un trabajo ya fallido se resuelve sin intentar iniciarlo otra vez. Un original interrumpido solo se considera transferido si IDG informa bytes durables. La autorización opcional de `downloads` sigue siendo gradual: el listener se instala tras concederla y se retira al revocarla. Las exclusiones por origen, MIME, tamaño mínimo y tamaño desconocido conservan una candidata permitida.

**Verificado automáticamente en esta reanudación:** las nuevas regresiones fallaron antes de las correcciones de duplicado y respuesta perdida y pasaron después. El recorrido real descubrió además que los eventos de progreso reconstruían los botones del popup durante un clic; la regresión de componente reprodujo el fallo y pasó tras conservar los nodos si el trabajo/propuesta no cambió. Otra regresión demostró que una actualización tardía borraba una exclusión `.exe` todavía sin guardar; el campo enfocado o modificado conserva ahora el borrador hasta que Guardar confirme. `test:unit` terminó con 37/37; `test:coverage` pasó sus umbrales sin cambios (TypeScript propio: 27,31 % líneas, 15,17 % funciones, 25,29 % ramas; `worker.ts`: 83,87 % líneas, 71,42 % funciones, 65,72 % ramas). `pnpm check`, `extension:build` y el shell Chromium sin permiso opcional pasaron. `scripts/Check.ps1` terminó con salida 0 antes de los ajustes del popup; después pasaron los tests y builds afectados, no se atribuye una nueva ejecución general a esos últimos ajustes. El arnés [Chromium/Tauri/HTTP](../scripts/test-chromium-capture.mjs), con host, runtime y perfiles aislados reales, pasó: Pausar/Reanudar desde el popup cambió el estado del trabajo y permitió completarlo con hash correcto; dos clics del enlace dieron una sola propuesta y un solo trabajo; una caída del motor **antes de aceptar** dejó completar el original en Chromium, y el reinicio/reconexión retiró la propuesta sin trabajo extra. El primer intento ampliado falló por interpretar como JSON la salida vacía de `idg-probe shutdown`; se corrigió solo el arnés y la repetición pasó. El registro temporal del host se restauró. Estas son interacciones automatizadas, no revisión humana.

**Regresión adicional y corte posterior a la aceptación:** una repetición del arnés mostró una segunda solicitud de enlace directo tras doble clic; los botones de enlace y de oferta seguían aceptando pulsaciones después de iniciar la primera. Dos regresiones de componente fallaron antes del cambio y pasaron al desactivar cada botón desde el primer clic. Con la extensión recompilada, el arnés completo volvió a pasar; el fallo intermitente previo queda registrado. Otro escenario aceptó `handoff-outage.bin` en IDG, detuvo el motor antes de recibir bytes durables y comprobó que el original de Chromium terminó con SHA-256 correcto. Tras reiniciar y reconectar, la solicitud pendiente se resolvió, conservó exactamente el ID del trabajo aceptado, no abrió otro original ni publicó un archivo IDG parcial. El fixture retiene solo el rango de ese caso hasta el corte. Esto verifica **pérdida del motor** después de aceptar, no un cierre forzado del proceso host. En el HEAD de este incremento pasaron `pnpm check`, `test:unit` (39/39), `test:coverage` (TypeScript propio: 544/1933 líneas, 28,14 %; 104/659 funciones, 15,78 %; 474/1854 ramas, 25,56 %), `extension:build` y el arnés Chromium/Tauri/HTTP real. Los mínimos y exclusiones de cobertura no cambiaron; `scripts/Check.ps1` general también terminó con salida 0 sobre este código final (formato, Clippy, Rust, tipos, builds y arneses de procesos/HTTP), sin ejecutar su matriz opcional `-Integration`. Los tres registros temporales del host se restauraron y verificaron.

**Pendientes concretos de fase 07:** comprobar en Chrome interactivo el diálogo real de concesión/rechazo del permiso opcional, el gesto del icono `activeTab` y el menú contextual; verificar la caída real del **proceso host** después de aceptar y la terminación real del service worker durante el traspaso, además del cierre de Chromium durante una transferencia; probar el tratamiento seguro de sesiones autenticadas/redirecciones antes de ampliar el GET público y repetible. El shell headless no acredita el diálogo; los arneses nuevos cubren cortes del motor antes y después de aceptar, no esas interacciones. La [PR #7](https://github.com/SrEdgarR/IDG/pull/7) permanece en borrador mientras estos casos puedan ocultar una pérdida o duplicado.

**Compatibilidad ampliada separada:** pruebas específicas de Edge, Windows 10, accesibilidad exhaustiva y DPI físico siguen pendientes; no se atribuyen a Chromium headless. La cancelación manual de energía simulada de fase 06 permanece **BLOQUEADA / PENDIENTE**, sin repetición ni acciones de energía. No se tocaron los dos perfiles temporales cuya limpieza fue rechazada.

**SIGUIENTE_PASO vigente:** revisar el CI del HEAD actualizado de la PR #7 y completar la matriz acotada de permiso interactivo, gesto, caída del proceso host y terminación del worker con perfiles aislados. Corregir solo defectos demostrados y mantener la PR #7 en borrador hasta entonces. No iniciar fase 08, fusionar PR #6/#7 ni publicar extensión o release.

## Cierre consolidado de pendientes esenciales de fase 07 — 2026-09-26

La rama de trabajo sigue siendo `feat/07-extension-chromium` y la PR #7 conserva su base `feat/06-colas-y-organizacion`. La PR #8 ya está integrada en esta rama; no se repitió su fusión. Las comprobaciones siguientes corresponden a los cambios de esta sección y se registran como automatización, no como revisión humana.

**Comprobado automáticamente:** `scripts/Check.ps1` pasó formato, Clippy estricto, Rust, exportación de tipos, TypeScript, pruebas Node/Vitest, builds de escritorio/extensión y arneses de runtime, HTTP y segmentación tras los cambios de lógica y arnés; `pnpm test:unit` pasó 41/41. `scripts/test-http-runtime.mjs` probó el fixture local con una redirección entre puertos/orígenes, una sesión ficticia sin cookies reales, un destino que devuelve HTML de acceso y un endpoint POST-only: las peticiones no llevaron `Cookie` ni `Authorization`; la descarga pública redirigida terminó con el hash esperado y las respuestas HTML fallaron como `representation` sin archivo publicado. `scripts/test-chromium-capture.mjs` pasó con Tauri/WebView2, Chromium for Testing, Native Messaging y runtime reales, datos y perfil aislados, y manifiesto de integración preconcedido solo para `downloads` y `127.0.0.1`. Después se aclaró el texto estático del popup y `pnpm extension:build` pasó sobre esa actualización; no se repitió la suite general por ese cambio de copy y documentación.

**Recuperación real comprobada:** una descarga explícita desde un enlace elegido se aceptó y persistió con cero bytes durables; se terminó el proceso Native Messaging hijo del perfil, el runtime conservó la misma identidad y, al pulsar «Reconectar», el host volvió a conectar. El mismo trabajo completó, sin solicitudes de rango nuevas al reconectar ni rangos repetidos, y su hash coincidió con el fixture. En otra descarga explícita el arnés terminó el service worker real mediante CDP mientras el trabajo seguía retenido; observó que desaparecía y reaparecía tras reconectar, con un solo trabajo, sin nuevas solicitudes de rango y con hash correcto. Para el cierre del navegador, esperó bytes durables en IDG, cerró Chromium mientras el trabajo seguía activo y comprobó que el runtime independiente terminó el mismo trabajo y publicó el hash esperado. El arnés también confirmó que una descarga observada por Chromium termina ahí y no genera propuesta, trabajo ni cancelación.

Los fallos iniciales de esta ampliación fueron del arnés y se corrigieron antes del pase final: el gate dejaba pasar la petición HTTP inicial sin `Range`; se esperaba una única solicitud aunque el motor segmenta en varios rangos; y las aserciones asumían que Playwright entregaría un evento de lifecycle o que CDP cambiaría el ID del target al reactivar el worker. La versión final bloquea la primera petición de cuerpo antes de responder y usa la presencia/ausencia del target y la unicidad de los rangos como evidencia. No se relajaron comprobaciones de integridad ni umbrales.

**No soportado con alternativa segura comprobada:** `chrome.downloads.DownloadItem` no expone método HTTP ni cuerpo. El worker ya no reproduce una descarga observada como GET, no la cancela ni la ofrece como traspasada; Chromium la completa. Para la ruta admitida, una persona elige un enlace directo, confirma que es un GET público repetible y acepta el diálogo en IDG. No se transfieren cookies de perfil, credenciales ni solicitudes POST; la página de acceso y el endpoint POST-only quedaron rechazados en los fixtures. Esto no comprueba una sesión autenticada real y no la marca como compatible.

**No comprobado por limitación del entorno:** el recorrido normal de pedir/conceder/rechazar el permiso opcional, abrir desde el icono con `activeTab` y usar el menú contextual. El test automatizado usa manifiesto de integración con permiso concedido de antemano y abre el popup como página de prueba; no acredita esos gestos. La superficie computer-use disponible no expuso una ventana de Chrome interactiva para resolver el diálogo, por lo que no se intentó eludir el bloqueo. Estos casos mantienen la PR #7 en borrador.
...
`Check.ps1` sin `-Integration` aprobó build, tipos, unitarias, Rust y suites generales descritas arriba. `-Integration` no se ejecutó: el registro no pertenece a esta copia, el permiso normal requiere interacción autorizada y la matriz no debe modificar otra instalación. El workflow de GitHub tampoco sustituye Tauri/WebView2 ni las pruebas reales de navegadores.

**CI de fase 08 por SHA:** el run de PR [36263985686](https://github.com/SrEdgarR/IDG/actions/runs/36263985686) y el run de push [36263982754](https://github.com/SrEdgarR/IDG/actions/runs/36263982754) corresponden exactamente a `e19b9428820e97ee0dd88e0dc2c1fae6ce0fc37c`. A las 18:58 UTC del 2026-09-26, portable y UI estaban aprobados; Windows seguía ejecutando `Suite Windows`. El run anterior de PR [36263598535](https://github.com/SrEdgarR/IDG/actions/runs/36263598535), sobre `e74a8a987c041311bca044b68822399159cb86bc`, falló en `Extensión en Chromium`: el verificador aún esperaba permiso opcional `downloads` y el control de captura automática, que están deliberadamente deshabilitados. `e19b942` actualizó sus aserciones al comportamiento requerido; el paso equivalente ya pasó en CI para ese SHA. Este workflow no verifica la transferencia Native Messaging real de Firefox ni los gestos de permisos normales.
...
**SIGUIENTE_PASO vigente:** cuando el propietario pueda probar el Native Messaging con un registro de desarrollo perteneciente a este checkout sin reemplazar el ajeno, ejecutar una vez el E2E Firefox y verificar archivo/hash. Después comprobar los gestos pendientes de permisos con el manifiesto normal en perfil aislado. Mantener PR #9 en borrador mientras esos casos esenciales sigan pendientes. No fusionar PR #6, #7 ni #9, y no avanzar a fase 09.
