...
**Verificado automáticamente en esta reanudación:** las nuevas regresiones fallaron antes de las correcciones de duplicado y respuesta perdida y pasaron después. `test:unit` terminó con 35/35; `test:coverage` pasó sus umbrales sin cambios (TypeScript propio: 23,45 % líneas, 13,74 % funciones, 24,13 % ramas; `worker.ts`: 83,87 % líneas, 71,42 % funciones, 65,72 % ramas). `pnpm check`, `extension:build` y el shell Chromium sin permiso opcional pasaron. El arnés [Chromium/Tauri/HTTP](../scripts/test-chromium-capture.mjs), con host, runtime y perfiles aislados reales, pasó: dos clics del enlace dieron una sola propuesta y un solo trabajo con hash correcto; una caída del motor **antes de aceptar** dejó completar el original en Chromium, y el reinicio/reconexión retiró la propuesta sin trabajo extra. El primer intento ampliado falló por interpretar como JSON la salida vacía de `idg-probe shutdown`; se corrigió solo el arnés y la repetición pasó. El registro temporal del host se restauró. Estas son interacciones automatizadas, no revisión humana.

**Pendientes concretos de fase 07:** comprobar en Chrome interactivo el diálogo real de concesión/rechazo del permiso opcional, el gesto del icono `activeTab` y el menú contextual; verificar pérdida del host **después** de aceptar y terminación real del service worker durante el traspaso, además del cierre de Chromium durante una transferencia; probar pausa/reanudar desde el popup y el tratamiento seguro de sesiones autenticadas/redirecciones antes de ampliar el GET público y repetible. El shell headless no acredita el diálogo; el arnés de pérdida anterior cubre solo la etapa previa a aceptar. La [PR #7](https://github.com/SrEdgarR/IDG/pull/7) permanece en borrador mientras estos casos puedan ocultar una pérdida o duplicado.
