...
**Verificado automáticamente en esta reanudación:** las nuevas regresiones fallaron antes de las correcciones de duplicado y respuesta perdida y pasaron después. El recorrido real descubrió además que los eventos de progreso reconstruían los botones del popup durante un clic; la regresión de componente reprodujo el fallo y pasó tras conservar los nodos si el trabajo/propuesta no cambió. `test:unit` terminó con 36/36; `test:coverage` pasó sus umbrales sin cambios (TypeScript propio: 27,10 % líneas, 15,09 % funciones, 25,13 % ramas; `worker.ts`: 83,87 % líneas, 71,42 % funciones, 65,72 % ramas). `pnpm check`, `extension:build` y el shell Chromium sin permiso opcional pasaron. `scripts/Check.ps1` terminó con salida 0 antes del ajuste del popup; después pasaron los tests y builds afectados, no se atribuye una nueva ejecución general a ese último ajuste. El arnés [Chromium/Tauri/HTTP](../scripts/test-chromium-capture.mjs), con host, runtime y perfiles aislados reales, pasó: Pausar/Reanudar desde el popup cambió el estado del trabajo y permitió completarlo con hash correcto; dos clics del enlace dieron una sola propuesta y un solo trabajo; una caída del motor **antes de aceptar** dejó completar el original en Chromium, y el reinicio/reconexión retiró la propuesta sin trabajo extra. El primer intento ampliado falló por interpretar como JSON la salida vacía de `idg-probe shutdown`; se corrigió solo el arnés y la repetición pasó. El registro temporal del host se restauró. Estas son interacciones automatizadas, no revisión humana.
