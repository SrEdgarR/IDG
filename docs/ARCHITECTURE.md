...
## Implementación de fase 04

`core/download/ranges` valida cobertura; `segmented` coordina workers HTTP y un escritor por archivo mediante canal acotado; `resources` comparte presupuestos globales/origen, tokens de velocidad y adaptación medida. `http` conserva el camino secuencial y pasa por los mismos presupuestos. El runtime admite múltiples trabajos bajo mutex y publica snapshots sin convertirse en un segundo motor. `active_requests` cuenta permisos HTTP reales, no sockets ni tareas esperando permiso.

La migración 002 agrega límites protegidos sin reescribir blobs antiguos; los campos nuevos se leen con defaults. Opciones, rangos y hashes por rango siguen en el documento DPAPI. El SHA final se lee sobre el archivo completo. Contrato aditivo v1/capacidades esquema 2; listado de rangos paginado. [ADR-012](decisions/012-segmentacion-recursos.md) y [guía](SEGMENTATION_DEVELOPMENT.md). Sin cambios de UI ni permisos de extensión.
